...
both <- rbind(importEssential("89B", "CBE"), importEssential("89A", "CBE"))
meta <- both %>% filter(Library == "89B")
...
p1 <- ggplot(both, aes(x = editRate + 0.001, color = four, linetype = paired) ) +
  ...
  scale_color_manual(values = c("TRUE_89A" = "red", "FALSE_89A" = "darkgrey", "FALSE_89B" = "firebrick", "TRUE_89B" = "black")) +
  scale_y_continuous(expand = c(0,0)) +
  ggtitle("CBE (n = 3.3M)")
cowplot::ggsave(p1, file= "output_figures/01_ecdf_CBE.pdf", width = 2.5, height = 1.6)

plot_df_tile <- meta %>% group_by(X1, X2) %>% 
  summarize(count_5 = sum(editRate >= 0.05), n = n(), avg_edit = mean(editRate)) %>%
  mutate(prop_5 = count_5/n)

# Get attributes for plot together
plot_df_tile$X2 <- factor(as.character(plot_df_tile$X2), levels = c("t", "g", "c", "a"))
max_val <- round(max(plot_df_tile$prop_5) + 0.05, 1)
n <- dim(meta)

p2 <- ggplot(plot_df_tile, aes(x = X1, y = X2, fill = prop_5)) +
  ...
  theme(legend.position = "none") + ggtitle("CBE")
cowplot::ggsave(p2, file= "output_figures/01b_tile-5percent_CBE.pdf", width = 1.1, height = 1.2)

max_val

# Demonstrate non-linearity
plot_df_tile2 <- meta %>% group_by(X1, X2, paired) %>% 
...
p3 <- ggplot(plot_df_tile2, aes(x = X1, y = X2, fill = fold_change)) +
  geom_tile() + pretty_plot(fontsize = 6) + L_border() + labs(x = "5' base", y = "3' base") +
  scale_y_discrete(expand = c(0,0), labels = rev(c("A", "C", "G", "U"))) +
  scale_x_discrete(expand = c(0,0), labels = c("A", "C", "G", "U")) +
  scale_fill_gradientn(colors = jdb_palette("brewer_blue"), limits = c(1,max_val2)) +
  theme(legend.position = "none") + ggtitle("CBE")
cowplot::ggsave(p3, file= "output_figures/01c_tile-FC_CBE.pdf", width = 1.1, height = 1.2)
