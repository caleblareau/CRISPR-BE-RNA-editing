...
model_dir <- "../model_outputs/CBE-19MAR"
...
all_data <- importForDeep("89B", "CBE")

# Randomly subset the data
idx <- sample(1:(dim(all_data)[1]), round(dim(all_data)*0.7))
...
CBE <- data.frame(predicted = inv.logit(c(training_prediction, validation_prediction)),
                  observed = inv.logit(c(training_labels, validation_labels))) %>% 
  filter(predicted > 0.01 | observed > 0.01)

cor(CBE)

smoothScatter(CBE[,1]*100, CBE[,2]*100,  xlim = c(0,35), ylim = c(0,35),
              colramp = colorRampPalette(c("white", jdb_palette("solar_rojos"))), pch = NA,
              xlab = "Deep - CBE - predicted", ylab = "Deep - CBE - observed")
